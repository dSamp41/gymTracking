...
def buildGraphOG(vals: list):
    '''
    Costruisce grafico di boxplot, vecchia versione
    val: list[list[int]]
    '''
    ...
    avgs = [np.mean(arr) for arr in vals]   #array delle medie

    ax1.boxplot(vals)
    ax1.plot(range(1, len(vals)+1), avgs)
    #return root
    root.mainloop()
